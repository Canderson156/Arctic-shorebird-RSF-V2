...
#30 out of 1152 plots were >1m apart. 
#maximum = 406m apart
...
prism_sep <- prism_polys_proj %>%
  filter(dist > as_units(0.01, "km"))

prism_sep <- merge(prism_sep, add, all.x = TRUE)


st_write(prism_sep, "exported/proj_seperated.shp")



check <- bigdata_raw %>%
  filter(Standardized_Plot_Name %in% prism_sep$Plot)

write.csv(check, "check.csv")

test <- prism_sep %>%
  filter(Plot == "ELI-0017A")
...
good_sep <- good_test %>%
  filter(Plot %in% prism_sep$Plot)

st_write(good_sep, "exported/good_seperated.shp")
